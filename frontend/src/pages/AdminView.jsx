...
export default function AdminView({ currentUser, onShowToast }) {
  ...
  const handleToggleUserStatus = async (userId, currentEnabled) => {
    try {
      ...
      onShowToast(`✓ Đã ${!currentEnabled ? 'kích hoạt' : 'tạm khóa'} tài khoản thành công!`);
      loadData();
    } catch (err) {
  ...
  const handleChangeRole = async (userId, newRole) => {
    try {
      ...
      onShowToast('⚠️ ' + (err.message || 'Lỗi phân quyền'));
    }
  };
  ...
  const handleToggleCompanyVerification = async (companyId, currentVerified) => {
    try {
      ...
      onShowToast('⚠️ ' + (err.message || 'Lỗi xác thực'));
    }
  };
  ...
  return (
    <div>
      <nav className="tabs-nav" role="tablist">
        ...
        >
          📜 Nhật ký Hệ thống & Audit Log ({auditLogs.length})
        </button>
      </nav>
                  ...
                  <tr key={u.id}>
                    ...
                    <td>
                      <span className={`badge ${u.enabled ? 'badge-success' : 'badge-danger'}`}>
                        {u.enabled ? '🟢 Đang hoạt động' : '🔴 Đã tạm khóa'}
                      </span>
                    </td>
                    <td>
                      <select
              ...
              <tbody>
                {companies.map((comp) => (
                  <tr key={comp.id}>
                    ...
                    <td>
                      {comp.website ? (
                        <a href={comp.website} target="_blank" rel="noreferrer" style={{ color: 'var(--primary)' }}>
                          {comp.website}
                        </a>
                      ) : '---'}
                    </td>
                    <td>
                      <span className={`badge ${comp.verified ? 'badge-success' : 'badge-warning'}`}>
                        {comp.verified ? '✓ Đã xác thực uy tín' : 'Chưa xác thực'}
                      </span>
                    </td>
                    <td>
                      <button
