...
5. Fallback: install dependencies and build if needed:
   ...
   cd "$DASHBOARD_DIR" && ((pnpm install 2>/dev/null || npm install 2>/dev/null || true))
   ```
   Then ensure the core package is built (the dashboard depends on it):
   ```bash
   : "${PLUGIN_ROOT:?Run step 3 first so PLUGIN_ROOT is set}"
   cd "$PLUGIN_ROOT" && (pnpm --filter @understand-anything/core build 2>/dev/null || (cd packages/core && npm run build 2>/dev/null) || npx tsc -p packages/core/tsconfig.json)
   ```
