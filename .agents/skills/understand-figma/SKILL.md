...
3. Resolve `PLUGIN_ROOT` and ensure core is built (same logic as `/understand` Phase 0.1.5). If `packages/core/dist/figma/index.js` is missing, run:
   ```bash
   cd "$PLUGIN_ROOT" && ((pnpm install 2>/dev/null || npm install 2>/dev/null || true)) && (pnpm --filter @understand-anything/core build 2>/dev/null || (cd packages/core && npm run build 2>/dev/null) || npx tsc -p packages/core/tsconfig.json)
   ```
